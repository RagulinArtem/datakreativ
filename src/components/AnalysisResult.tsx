...
function ElementBar({ score, label, details }: { score: number; label: string; details: string }) {
  const colors = getScoreColor(score);
  return (
    <div className="glass-card p-5 transition-all duration-300 hover:border-white/10">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-sm font-medium">{label}</span>
        <span className={`text-sm font-bold tabular-nums ${colors.label}`}>{score}</span>
      </div>
...
export default function AnalysisResult({ result, imageUrl, onReset }: Props) {
  return (
    <div className="page-transition space-y-8">
      {/* Top section: Score + Image */}
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <ElementBar {...result.elements.composition} />
        </div>
      </div>
